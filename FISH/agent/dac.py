...
class DACAgent:
	def __init__(self, obs_shape, action_shape, device, lr, feature_dim,
				 ...
				 augment, use_actions, suite_name, obs_type, bc_weight_type, offset_mask):
		self.device = device
		self.lr = lr
		...
		self.actor = Actor(repr_dim, action_shape, feature_dim,
						   hidden_dim, offset_mask, device).to(device)
	...
	def act(self, obs, step, eval_mode):
		...
		return action.cpu().numpy()[0], action.cpu().numpy()[0]
	...
	def dac_rewarder(self, obses, actions):
		...
		obses = self.encoder(obses) if self.use_encoder else obses
		if self.use_actions:
