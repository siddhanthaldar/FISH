...
from replay_buffer_robot import ReplayBufferStorage, make_replay_loader, make_expert_replay_loader
...
class WorkspaceIL:
	def __init__(self, cfg):
		...
		elif repr(self.agent) == 'vinn':
			self.cfg.suite.num_train_frames = self.cfg.num_train_frames_vinn
			self.cfg.suite.num_seed_frames = 0
		...
		while train_until_step(self.global_step):
			...
			if repr(self.agent) not in ['bc', 'vinn']:
				...
				if eval_every_step(self.global_step):
					...
					self.eval()
					self.timer.eval()
					self.save_snapshot(name=f"snapshot_eval_{self.global_step}.pt")
	...
	def save_snapshot(self, name=None):
		snapshot = self.work_dir / ('snapshot.pt' if name is None else name)
