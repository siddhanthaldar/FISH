...
class RobotKeyInsertionEnv(robot_env.RobotEnv):
	def __init__(self, height=84, width=84, step_size=10, enable_arm=True, enable_gripper=True, enable_camera=True, camera_view='side',
				 use_depth=False, dist_threshold=0.03, random_start=False, x_limit=None, y_limit=None, z_limit=None, pitch=0, roll=180, keep_gripper_closed=True):
		robot_env.RobotEnv.__init__(
			self,
			home_displacement=[2.64, -0.25, 1.17],
			height=height,
			width=width,
			step_size=step_size,
	...
	def get_random_pos(self, pos=None):
		x_disp = self.arm.x_limit[0] + np.random.rand()*(self.arm.x_limit[1] - self.arm.x_limit[0])
		y_disp = self.arm.y_limit[0] + np.random.rand()*(self.arm.y_limit[1] - self.arm.y_limit[0])
		z_disp = self.arm.z_limit[0] + np.random.rand()*(self.arm.z_limit[1] - self.arm.z_limit[0])
		
		if pos is None:
			pos = np.zeros(3).astype(np.float32)
		pos[0] = x_disp
		pos[1] = y_disp 
		pos[2] = z_disp if not self.arm.highest_start else self.arm.z_limit[1]
	...
	def step(self, action):
		...
		new_pos[:3] += action[:3] * self.dist_threshold
		if self.enable_arm:
